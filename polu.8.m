...
print_poly([]) --> [].
print_poly([C]) --> print_comp(C).
print_poly([C1, C2 | CC]) -->
	print_comp(C1),
	print(" + "),
	print_poly([C2 | CC]).

:- pred print_comp(comp::in, io::di, io::uo).
print_comp(comp(K, L)) --> print(K), print_lvp(L).

:- pred print_lvp(lvp::in, io::di, io::uo).
print_lvp([]) --> [].
print_lvp([var(V, P) | T]) -->
	print(V),
	print("^"),
...
main -->
	{
	 ...
	 %R = pwr_exp([one, comp(1,[var(x,1)])],40)
	 R = pwr_exp([one, simple(x,1), simple(y, 1), simple(z, 1)], 20),
	 R1 = pwr([one, simple(x,1), simple(y, 1), simple(z, 1)], 20),
	 Eq = (R=R1 -> "equal"; "not equal") % equal
	},
	%print(P),nl,
	%print(Q),nl,
	print_poly(R),nl,
