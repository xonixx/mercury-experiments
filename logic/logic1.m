...
:- typeclass unifiable_with_free(T) <= unifiable(T) where [
	func free_var = T
].

:- instance unifiable(maybe(T)).
:- instance unifiable_with_free(maybe(T)).
...
:- import_module maybe, list, require.

:- instance unifiable_with_free(maybe(T)) where [
...
%head_tail(H0, H, T0, T, [H1 | T1], [H, T]) :- H=unify(H0, H1), T=unify(T0, T1).